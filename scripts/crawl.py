...
class Crawler:
    def __init__(
        ...
        if self.data_store.server_type == "local":
            requests.get(
                f"http://{self.data_store.hostname}:5555/api/workers?refresh=1"
            )
        ...
        for url in urls:
            ...
            while True:
                if self.data_store.server_type == "local":
                    ...
                    req = requests.get(
                        f"http://{self.data_store.hostname}:5555/api/tasks?state=RECEIVED"
                    )
                    if req.status_code != 200:
                        raise Exception(
                            f"Failed to get workers from celery api. Status code: {req.status_code}"
                        )
                    else:
                        tasks = req.json()
                        no_of_tasks = 0
            ...
            if "submission_id" in r.json():
                submission_id = r.json()["submission_id"]
                submission_identifiers.append((submission_id, url, datetime.now()))
            else:
                submission_id = r.json().get(
                    "submission_id", f"generated_{datetime.now().timestamp()}"
                )
                submission_identifiers.append((submission_id, url, datetime.now()))
