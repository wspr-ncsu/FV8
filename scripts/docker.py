...
def create(data_directory: str, instance_count: int):
    pull_proc = sbp.run(
        ["docker", "pull", "visiblev8/vv8-base:latest"], cwd=data_directory
    )
    if pull_proc.returncode != 0:
        print("Failed to pull latest images for visiblev8 for vv8-crawler server")
        os._exit(-1)
    pull_proc = sbp.run(
        ["docker", "pull", "sohomdatta1/visiblev8-postprocessors:latest"],
        cwd=data_directory,
    )
    if pull_proc.returncode != 0:
        print(
            "Failed to pull latest images for visiblev8 postprocessors for vv8-crawler server"
        )
        os._exit(-1)
...
def docker_parse_args(docker_arg_parser: argparse.ArgumentParser):
    docker_arg_parser.add_argument(
        "-s", "--start", help="start the vv8-crawler server", action="store_true"
    )
    docker_arg_parser.add_argument(
        "-t", "--stop", help="stop the vv8-crawler server", action="store_true"
    )
    docker_arg_parser.add_argument(
        "-r", "--rebuild", help="rebuild the vv8-crawler server", action="store_true"
    )
    docker_arg_parser.add_argument(
        "-f",
        "--follow-logs",
        help="follow the logs of the vv8-crawler server",
        action="store_true",
    )
